...
public class BluetoothMgr extends Activity {
    ...
    public SendBTStatus send_status = SendBTStatus.NONE;
    	
    	
   static public enum SendBTStatus {
    	NONE,
        SEARCH,
        DEVICETYPE,
        OUTPUTENERGY,
    ...
    private void setupChat() {
        ...
        mConversationView = (ListView) findViewById(R.id.searchlist);
        ...
        mConversationView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView parent, View v, int position, long id)
            {
            	...
            	intent.setClass(BluetoothMgr.this, SplashActivity.class);
            	byte getid = stack.get(position);
            	intent.putExtra("saddr", Byte.toString(getid));
        ...
        mSendButton = (Button) findViewById(R.id.it_search);
        mSendButton.setOnClickListener(new OnClickListener() {
            public void onClick(View v) 
            {
            	...
            	stack.clear();
            	send_status = send_status.SEARCH;
                ...
                new Thread()
                {
                  public void run()
                  {
                    for (gcount=1; gcount<2; gcount++)
                  	{
                  		try {
                      		String message = "a," + Integer.toHexString( gcount ) + ",3,0,77,0,8,f4,16,d";
                      		Log.i(TAG, message);
                      		sendMessage(message);
							Thread.sleep(1000);
...
/*
        	String crc = String.format("0x%X", CRC16(crcbuf));
      		Log.i(TAG, "crc check:" + crc);
 */
      		int value = CRC16(crcbuf);
      		send[7] = (byte)( (value >> 8) & 0xFF );
      		send[8] = (byte)( value & 0xFF );
/*        	
...
/*                        
            ...
            case MESSAGE_READ:
                ...
                String rMessage = readBuf[1] + " " + readMessage;
            	stack.add(readBuf[1]);
            	if (send_status == send_status.SEARCH)
                    mConversationArrayAdapter.add(rMessage);
